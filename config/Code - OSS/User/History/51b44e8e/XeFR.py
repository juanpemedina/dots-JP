...
budget_columns_to_keep = ['Cliente', 'ENGAGEMENT ID', 'USUARIO', 'HORAS PRESUPUESTADAS' ]
df_budget_cleaned = df_budget.loc[:, budget_columns_to_keep].copy()

df_budget_cleaned.rename(columns={'ENGAGEMENT ID': 'Engagement ID', 'ENGAGEMENT ID': 'Engagement ID','HORAS PRESUPUESTADAS': 'Budgeted Hours', 'USUARIO': 'Employee'}, inplace=True)

with pd.ExcelWriter('cleaned_dataPEPE.xlsx') as writer:
    df_budget_cleaned.to_excel(writer, sheet_name='DataLimpiaASS', index=False)
