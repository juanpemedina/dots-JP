...
columns_to_keep_ASS = [col for col in COLUMNS_TO_KEEP if col in df.columns]
df_cleaned_ASS = df.loc[:, columns_to_keep].copy()
# Convertir la columna 'Accounting Cycle Date' a tipo datetime
...
df_cleaned_merged = pd.merge(df_cleaned, df_budget_cleaned, on='Engagement ID', how='left')



with pd.ExcelWriter('cleaned_dataPPPP.xlsx') as writer:
    df_cleaned.to_excel(writer, sheet_name='DataLimpia', index=False)
    df_cleaned_ASS.to_excel(writer, sheet_name='DataLimpiaASS', index=False)
