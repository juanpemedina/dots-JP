...
budget_columns_to_keep = ['ENGAGEMENT ID', 'USUARIO' 'HORAS PRESUPUESTADAS' ]
df_budget_cleaned = df_budget.loc[:, budget_columns_to_keep].copy()

df_budget_cleaned.rename(columns={'ENGAGEMENT ID': 'Engagement ID', 'HORAS PRESUPUESTADAS': 'Budgeted Hours', 'USUARIO': 'Employee'}, inplace=True)
