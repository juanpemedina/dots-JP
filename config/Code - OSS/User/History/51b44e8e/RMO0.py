...
df_grouped_sum_detailed = df_hours_cleaned.groupby(
...
# Adjust the budget DataFrame column names to match
df_budget.columns = ['Client', 'Engagement ID', 'Employee', 'Budgeted Hours', 'Budgeted Hours File', 'Unused']

# Merge the budget data into the grouped data
df_merged = df_grouped_sum_detailed.merge(df_budget[['Engagement ID', 'Employee', 'Budgeted Hours']], how='left', on=['Employee', 'Engagement ID'])
