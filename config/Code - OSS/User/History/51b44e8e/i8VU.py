...
# Read the hours table starting from the correct header row (ROW 6)
df_hours = pd.read_excel(file_path, sheet_name=sheet_name_hours, header=6)

# Read the budget table (adjust header if needed)
...
print(df_hours.columns.tolist())  # DELETE AFTER
print(df_budget.columns.tolist())  # DELETE AFTER

# Define the correct columns to keep based on actual column names
# Adjust these names based on the output of the above
columns_to_keep = [
...
columns_to_keep_hours = [col for col in columns_to_keep_hours if col in df_hours.columns]

# Filter the hours DataFrame to keep only the specified columns
df_hours_cleaned = df_hours.loc[:, columns_to_keep_hours].copy()

# Drop rows where the 'Employee' column is empty
df_hours_cleaned.dropna(subset=['Employee'], inplace=True)

# Group by Employee and Employee GPN
df_employee_gpn = df_hours_cleaned[['Employee', 'Employee GPN']].drop_duplicates()

# Group by Engagement and Engagement ID
df_engagement_id = df_hours_cleaned[['Engagement', 'Engagement ID']].drop_duplicates()

# Group by required columns and sum the 'Hours'
df_grouped_sum = df_hours_cleaned.groupby(
...
df_grouped_sum_detailed = df_grouped_sum_detailed.sort_values(by=['Accounting Cycle Date','Engagement'])

# Merge the budget data into the grouped data
# Adjust the column names as necessary based on the actual names in df_budget
df_merged = df_grouped_sum_detailed.merge(df_budget, how='left', left_on=['Employee', 'Engagement ID'], right_on=['Employee', 'Engagement ID'])

# Save the cleaned DataFrame and the grouped DataFrames to a new Excel file
with pd.ExcelWriter('cleaned_data.xlsx') as writer:
    df_hours_cleaned.to_excel(writer, sheet_name='Cleaned_Data', index=False)
    ...
    df_grouped_sum_detailed.to_excel(writer, sheet_name='SumaEngagement_Employee', index=False)  # falta hours left y persupuestadas
    df_engagement_sum.to_excel(writer, sheet_name='EngagementID_Sum', index=False)  # falta hours left y persupuestadas
    df_merged.to_excel(writer, sheet_name='GroupedSum_Detailed', index=False)