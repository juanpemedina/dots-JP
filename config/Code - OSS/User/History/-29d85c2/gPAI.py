...
df_sum_hours = df_grouped_sum.groupby(['Engagement ID', 'Engagement'])['Hours'].sum().reset_index()
...
with pd.ExcelWriter('cleaned_dataPPPP.xlsx') as writer:
    ...
    df_grouped_sum.to_excel(writer, sheet_name='SumaEngagement_Employee', index=False)
