...
with pd.ExcelWriter('cleaned_dataPPPP.xlsx') as writer:
    ...
    df_sum_hours.to_excel(writer, sheet_name='SumaHoras_EngagementEmployee', index=False)
    df_engagement_sum.to_excel(writer, sheet_name='HorasTotalesEngagement', index=False)
    df_sum_hours_weekly.to_excel(writer, sheet_name='GroupedSum_Detailed', index=False)
