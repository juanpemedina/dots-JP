describe('Prueba de Préstamos', () => {
  ...
  beforeEach(() => {
    ...
    login();
    cy.visit('http://localhost:5173/#/movimientos'); // Navigate to the movements page
    cy.contains('Préstamos').as('prestamosTab');
    cy.get('@prestamosTab').should('be.visible'); // Ensure the element is visible
    cy.get('@prestamosTab').click().click(); // Click on the element
  });
