...
df_cleaned_concat = pd.concat([df_cleaned, df_cleaned_ASS], ignore_index=True)




""" Agrupa Employee+GPN+Competency, Engagement+ID, y la suma de horas"""
df_sum_hours = df_cleaned_concat.groupby(
...
df_sum_hours_weekly = df_cleaned_concat.groupby(
...
df_engagement_with_budget = pd.merge(df_cleaned_concat, df_budget_cleaned, on='Engagement ID', how='left')
