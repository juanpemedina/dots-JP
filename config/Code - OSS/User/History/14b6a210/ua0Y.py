...
def process_file(file_path):
    ...
    ## Agregar if is external 
    df_cleaned_final['Engagement Type'] = df_cleaned_final['Engagement ID'].apply(lambda x: 'External' if str(x).startswith('E') else 'Internal')
    ...
    df_sum_hours = df_cleaned_final.groupby(['Employee', 'Employee GPN', 'Rank Description', 'Employee Service Line', 'Employee Sub Service Line', 'Employee Competency', 'Engagement', 'Engagement ID', 'Engagement Type', 'Client ID', 'Client Name'])['Hours'].sum().reset_index()
    ...
    df_engagement_sum = df_sum_hours.groupby(['Engagement ID', 'Engagement', 'Engagement Type'])['Hours'].sum().reset_index()
    ...
    """ Agrupa Employee+GPN+Competency, Engagement+ID, y la suma de horas según el Accounting Cycle Date """
    ...
    df_sum_hours['Remaining Hours'] = df_sum_hours['Budgeted Hours'] - df_sum_hours['Hours']
    df_engagement_sum['Remaining Hours'] = df_engagement_sum['Budgeted Hours'] - df_engagement_sum['Hours']
    df_sum_hours_weekly['Remaining Hours'] = df_sum_hours_weekly['Budgeted Hours'] - df_sum_hours_weekly['Hours']

    """ Guardar en Excel """
    # Guardar los DataFrames limpios y agrupados en un nuevo archivo de Excel
    with pd.ExcelWriter('cleaned_data_new1.xlsx') as writer:
        df_cleaned_final.to_excel(writer, sheet_name='DataLimpia', index=False)
        df_budget_cleaned.to_excel(writer, sheet_name='Data_Presupuesto', index=False)
        df_budget_cleaned_sum.to_excel(writer, sheet_name='Presupuesto_Sumado', index=False)
    ...
    print("Proceso completado y datos guardados en 'cleaned_data_new1.xlsx'")
