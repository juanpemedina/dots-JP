...
""" Constantes """
#FILE_PATH = 'Cargabilidad_Horas disponibles en proyecto_ CNS & ASS_2024.xlsx'
...
COLUMNS_TO_KEEP = [
    'Employee', 'Employee GPN', 'Rank Description', 'Employee Service Line', 'Employee Sub Service Line', 'Employee Competency', 'Engagement ID',
    'Engagement', 'Client ID','Client Name', 'Accounting Cycle Date', 'Transaction Date', 'Hours'
]
...
df_sum_hours = df_cleaned_final.groupby(['Employee', 'Employee GPN', 'Employee Competency', 'Engagement', 'Engagement ID', 'Client Name'])['Hours'].sum().reset_index()
...
with pd.ExcelWriter('cleaned_dataPres.xlsx') as writer:
