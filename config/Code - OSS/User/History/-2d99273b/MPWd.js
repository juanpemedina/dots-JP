describe('Historia de usuario 3. Prueba de campo de búsqueda', () => {  
    before(() => {
        ...
        // Capture the loan ID for later cleanup
        cy.url().then(url => {
            const urlParts = url.split('/');
            loanId = urlParts[urlParts.length - 1]; // Assuming the loan ID is at the end of the URL
        });
      ...
      after(() => {
        // Clean up the loan registered in the before hook
        if (loanId) {
            cy.request({
                method: 'DELETE',
                url: `/api/loans/${loanId}`, // Replace with the actual API endpoint for deleting a loan
                headers: {
                    'Authorization': 'Bearer ' + window.localStorage.getItem('token') // Assuming JWT auth
                }
            }).then((response) => {
    ...
    // Sorting test
    it.skip("Tabla ordenada descendentemente por fecha", () => {
