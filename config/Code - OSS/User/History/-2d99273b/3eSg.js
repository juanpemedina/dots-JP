describe('Historia de usuario 3. Prueba de campo de búsqueda', () => {  
    ...
    // Sorting test
    it.only("Debería ordenar la tabla descendentemente por fecha", () => {
        cy.visit("/#/movimientos");
        cy.wait(1000);
        cy.get("table tbody tr td:nth-child(3)")
        .then(($cells) => {
            // Extraer los textos de las celdas
            const dates = [...$cells].map(cell => cell.innerText.trim());
            // Convertir los textos a objetos Date
            const dateObjects = dates.map(date => new Date(date));
            // Verificar que las fechas estén en orden descendente
            const isDescending = dateObjects.every((date, i, arr) => {                return i === 0 || date >= arr[i - 1];
            });
